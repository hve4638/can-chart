...
class EasyDB:
    def __init__(self, filename=':memory:', verbose=False):
        ...
        self.closed = False
        if verbose:
            print(f'Connect to DB \'{filename}\'')
    ...
    def create_table(self, table:str, if_not_exists=False)->TableBuilder:
        if if_not_exists:
            query = f'CREATE TABLE IF NOT EXISTS {table}'
        else:
            query = f'CREATE TABLE {table}'

        return TableBuilder(self.cursor(), query, verbose=self.verbose)
